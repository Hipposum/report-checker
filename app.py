...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📥 Загрузить данные", "✉️ Сообщения", "📤 Отправить", "📚 История", "📊 Статистика", "✏️ Работа"])
...
# ─────────────────────────────────────────────────────────────────────────────
#  TAB 6 — РАБОТА (очередь открытых ошибок по преподавателям)
# ─────────────────────────────────────────────────────────────────────────────

with tab6:
    st.subheader("✏️ Работа")

    _work_records = load_history()
    _work_open_statuses = ("open", "message_sent", "pass_set")
    _work_all_open = [r for r in _work_records if r["status"] in _work_open_statuses]

    if not _work_all_open:
        st.success("🎉 Нет открытых ошибок — всё обработано!")
    else:
        # ── Фильтры ───────────────────────────────────────────────────────────
        _wf1, _wf2, _wf3 = st.columns([2, 2, 2])

        with _wf1:
            _w_periods = sorted(
                set((r["period_from"], r["period_to"]) for r in _work_all_open),
                key=lambda x: x[0], reverse=True,
            )
            _w_period_labels = ["Все периоды"] + [
                f"{fmt_date(pf)} — {fmt_date(pt)}" for pf, pt in _w_periods
            ]
            _w_sel_period = st.selectbox(
                "Период", _w_period_labels, key="work_period",
                label_visibility="collapsed",
            )

        with _wf2:
            _w_err_filter = st.selectbox(
                "Тип", ["Все ошибки", "Нет отчёта", "Нет комментария"],
                key="work_err_type", label_visibility="collapsed",
            )

        with _wf3:
            _w_intern_filter = st.selectbox(
                "Кто", ["Все", "Только стажёры", "Только основные"],
                key="work_intern", label_visibility="collapsed",
            )

        # Apply filters
        _work_filtered = _work_all_open
        if _w_sel_period != "Все периоды":
            _pidx = _w_period_labels.index(_w_sel_period) - 1
            _wpf, _wpt = _w_periods[_pidx]
            _work_filtered = [r for r in _work_filtered
                              if r["period_from"] == _wpf and r["period_to"] == _wpt]
        if _w_err_filter == "Нет отчёта":
            _work_filtered = [r for r in _work_filtered if r["error_type"] == "no_report"]
        elif _w_err_filter == "Нет комментария":
            _work_filtered = [r for r in _work_filtered if r["error_type"] == "no_abs_comment"]
        if _w_intern_filter == "Только стажёры":
            _work_filtered = [r for r in _work_filtered
                              if _teacher_info.get(r["teacher"], {}).get("is_intern")]
        elif _w_intern_filter == "Только основные":
            _work_filtered = [r for r in _work_filtered
                              if not _teacher_info.get(r["teacher"], {}).get("is_intern")]

        # ── Топ-метрики ───────────────────────────────────────────────────────
        _wt1, _wt2, _wt3 = st.columns(3)
        _w_teachers_open = set(r["teacher"] for r in _work_filtered)
        _w_interns_open  = sum(1 for t in _w_teachers_open
                               if _teacher_info.get(t, {}).get("is_intern"))
        _wt1.metric("📋 Открытых ошибок",   len(_work_filtered))
        _wt2.metric("👩‍🏫 Преподавателей",    len(_w_teachers_open))
        _wt3.metric("🎓 Из них стажёры",     _w_interns_open)

        st.divider()

        # ── Группируем по преподавателю ───────────────────────────────────────
        _work_by_teacher = {}
        for r in _work_filtered:
            _work_by_teacher.setdefault(r["teacher"], []).append(r)

        for teacher in sorted(_work_by_teacher.keys()):
            t_records = _work_by_teacher[teacher]
            t_info    = _teacher_info.get(teacher, {})
            is_intern = t_info.get("is_intern", False)
            subject   = t_info.get("subject", "")

            # Group by error type for summary
            no_rep = [r for r in t_records if r["error_type"] == "no_report"]
            no_cmt = [r for r in t_records if r["error_type"] == "no_abs_comment"]

            with st.container(border=True):
                # Header row
                _th1, _th2 = st.columns([4, 2])
                with _th1:
                    _ib = intern_badge(teacher, _teacher_info)
                    st.markdown(f"**{teacher}**&nbsp;&nbsp;{_ib}", unsafe_allow_html=True)
                    if subject:
                ...
                with _th2:
                    ...
                    parts = []
                    if no_rep:
                        total_lessons = sum(r.get("count", 0) for r in no_rep)
                        parts.append(f"📋 {len(no_rep)} дат · {total_lessons} занятий")
                    if no_cmt:
                        parts.append(f"💬 {len(no_cmt)} пропусков")
                    for p in parts:
                        st.caption(p)
                ...
                if no_rep:
                    dates_str = " · ".join(
                        f"{fmt_date(r['date'])} ({r.get('count',0)} шт.)"
                        for r in sorted(no_rep, key=lambda x: x["date"])
                    )
                    st.caption(f"🗓 {dates_str}")

                st.write("")

                # Action row
                _ta1, _ta2, _ta3, _ta4 = st.columns([3, 1, 1, 1])
                with _ta1:
                    _w_comment = st.text_input(
                        "Комментарий",
                        placeholder="Добавьте комментарий…",
                        key=f"wc_{teacher}",
                        label_visibility="collapsed",
                ...
                with _ta2:
                    if st.button("✅ Всё", key=f"wh_{teacher}", use_container_width=True,
                                 type="primary"):
                        for r in t_records:
                            update_history_record(r["id"], "handled", _w_comment, reviewer_name)
                        st.rerun()
                with _ta3:
                    if st.button("⚪ Пропустить", key=f"ws_{teacher}", use_container_width=True):
                        for r in t_records:
                            update_history_record(r["id"], "skipped", _w_comment, reviewer_name)
                        st.rerun()
                with _ta4:
                    # Individual records toggle
                    with st.popover("🔍", use_container_width=True):
                        for r in sorted(t_records, key=lambda x: x["date"]):
                            rid = r["id"]
                            err_icon = "📋" if r["error_type"] == "no_report" else "💬"
                            try:
                                d_str = datetime.strptime(r["date"], "%Y-%m-%d").strftime("%d.%m.%Y")
                            except Exception:
                                d_str = r["date"]
                            pill_cls = _STATUS_PILL.get(r["status"], "pill")
                            s_label  = _STATUS_META.get(r["status"], ("?",))[0]
                            st.markdown(
                                f"{err_icon} **{d_str}** — {r.get('count',0)} шт. "
                                f'&nbsp;<span class="{pill_cls}">{s_label}</span>',
                                unsafe_allow_html=True,
                            )
                            _ic1, _ic2 = st.columns(2)
                            if _ic1.button("✅", key=f"wih_{rid}", use_container_width=True):
                                update_history_record(rid, "handled", _w_comment, reviewer_name)
                                st.rerun()
                            if _ic2.button("⚪", key=f"wis_{rid}", use_container_width=True):
                                update_history_record(rid, "skipped", _w_comment, reviewer_name)
                                st.rerun()
            # Export
